...
show in a separate frame.  Can also be \\='(buframe FN-POS
...
If it is a string, the command is used as is.  Otherwise, it is expected
to be a function that returns the command as a string.
...
(defun preview-dvi*-process-setup (img-type start sentinel)
  ...
  (let (filename queued oldfiles snippet)
    ...
    (if (setq preview-gs-queue (nreverse preview-gs-queue))
        (progn
          ...
          (dolist (ov preview-gs-queue)
            (setq snippet (aref (overlay-get ov 'queued) 2))
            ;; Only add `preview-ps-file' if it doesn't exists. Also,
            ;; delete any files before overwriting 'filesnames.
            (if preview-ps-file
...
(defun preview-point-refresh (ov)
...
(defun preview-point-toggle-auto-refresh (enable)
  "Enable auto refresh of previews.
When ENABLE is nil, disable auto-refresh instead.  If called
interactively, auto-refreshing is toggled."
