    ...
    (let ((args arglist))
      (while args
        (let ((arg (car args)))
          (if (eq arg '&delay)
              (progn
                (setq args (cdr args))   ;; Skip next
                (setq delay (car args))
                (unless delay
                  (error "&delay requires a delay value")))
            (push arg real-args)))
        (setq args (cdr args))))
    (setq real-args (nreverse real-args))
    (unless delay
      (message "Warning: No &delay specified in debounced function %s"
               (symbol-name name)))
    `(progn
